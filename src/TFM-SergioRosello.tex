...
Una vez observada la cultura DevOps, es de esperar que se quiera seguir explorando hasta donde puede llegar el cambio de paradigma que ha traído esta filosofía de desarrollo y comunicación.
Al igual que ha surgido la vertiente DevSecOps, también ha surgido la vertiente DataOps.
Ambas representan la misma idea.
Romper los problemas de comunicación entre los distintos equipos que componen un producto y proporcionar al desarrollador con las herramientas necesarias para que pueda desempeñar su trabajo sin tener que lidiar con procesos, cambios de requisitos no documentados, y cualquier problema que se interponga entre el desarrollo y el producto final.

\subsection{Que es la seguridad?}
 
La seguridad en el mundo del software viene heredada del mundo físico, en el que primero se consolida un producto, y luego se tiene en cuenta su seguridad.
En los años <++> cuando Internet era mayoritariamente estático, o no estaba disponible a muchos usuarios, la seguridad era una pieza del puzzle que no estaba contemplada.
Los desarrolladores implementaban las funcionalidades sin pensar en como otra gente pudiese romper la aplicacion para sacar provecho del sistema en el que corre, ya sea bancario, reservas de plazas en el cine, o obtener información sensible.
A medida que Internet se ha extendido, hemos aprendido a la fuerza que hay gente que prefiere hacer uso de los errores de otros para sacar beneficio personal.

\subsection{Seguridad en la metodología ágil - Prevención contra el hacking}
...
Por eso, existen nuevas iniciativas llamadas ThreatBountyPorgrams <++>.
Estas iniciativas por parte de las empresas intentan convencer a los hackers a que les comuniquen como han conseguido entrar en el sistema, ofreciéndoles compensación económica.
Pero, esto no deja de ser una solucion a un problema que viene de antes, desde la fase de desarrollo.

\subsection{El problema en a seguridad}

El problema de la seguridad es que en la fase de desarrollo de software corriente, la seguridad reside a la derecha del todo, en la fase de mantenimiento del mismo.

%TODO: Anadir figura de proceso de desarrollo de software.
\begin{tikzpicture}[node distance=2cm]
  %Nodes
  \node (RA) [process] {Requirement analysis};
  \node (P) [process, right of=RA, xshift=2cm] {Planning};
  \node (SDD) [process, right of=P, xshift=2cm] {Software Design and development};
  \node (T) [process, right of=SDD, xshift=2cm] {Testing};
  \node (D) [process, right of=T, xshift=2cm] {Deployment};
  \node (M) [process, right of=D, xshift=2cm] {Maintenance};
  %Arrows
  \draw [arrow] (RA) -- (P);
  \draw [arrow] (P) -- (SDD);
  \draw [arrow] (SDD) -- (T);
